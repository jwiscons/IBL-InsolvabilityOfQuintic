...
Then the map $\hat{\mathcal{E}}_\alpha: F[x]/(m(x)) \rightarrow F(\alpha)$ defined via $\hat{\mathcal{E}}_\alpha(p(x) + (m(x))) =  p(\alpha)$ is an isomorphism. Thus $F[x]/(m(x)) \cong F(\alpha)$.
\end{theorem}
