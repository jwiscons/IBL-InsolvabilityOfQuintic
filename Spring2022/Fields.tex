...
When we plot points, there are different coordinate systems we could use. It turns out that  rectangular coordinates are good for adding complex numbers, but polar coordinates are better for multiplication. This leads to the following definition.
...
We arrive at an extremely important definition.
...
\begin{problem}
Plot each number in the same complex plane: $\zeta_2$, $\zeta_3$, $\zeta_4$, $\zeta_5$, $\zeta_6$.
\end{problem}

\begin{problem}
Plot each number in the same complex plane: $\zeta_6$, $(\zeta_6)^2$, $(\zeta_6)^3$, $(\zeta_6)^4$, $(\zeta_6)^5$, $(\zeta_6)^6$.
\end{problem}
...
Notice that this definition refers to ``\emph{an} $n^\text{th}$ root of $a$'' as opposed to ``\emph{the} $n^\text{th}$ root of $a$''. We will see, for example, that although there is only one cube root of $8$ in $\mathbb{R}$, there are three cube roots of $8$ in $\mathbb{C}$.
...
\begin{remark}\label{rmk.GenerateField}
