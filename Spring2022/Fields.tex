...
\item The \textbf{argument} of $z$, denoted $\Arg(z)$, is the \emph{angle} in the interval $[0,2\pi)$ of the point $(a,b)$ when written in polar coordinates. Thus, if $z\neq 0$, then $\cos \theta = \frac{a}{|z|}$ and $\sin \theta = \frac{b}{|z|}$. The argument of  $0$ is undefined.
\end{enumerate}
\end{definition}
...
\begin{problem}
Find \emph{all} roots of $x^3 - 8 = 0$, and also find \emph{all} roots of  $x^5 + 7 = 0$.
\end{problem}
...
It turns out that the group $S$ from the previous problem is isomorphic to the group $\operatorname{SU}(2)$ (one of the the so-called special unitary groups), which is quite important in theoretical physics. If you want to learn more, you can start on \href{https://en.wikipedia.org/wiki/Special_unitary_group#The_group_SU(2)}{Wikipedia}.
