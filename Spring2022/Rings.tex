...
If $I$ is any ideal of $\mathbb{Z}$, then $I$ is a principal ideal. Moreover, if $I$ is not the zero ideal, then $I = (d)$ if and only if $d$ has the smallest possible absolute value among all nonzero elements of $I$.
\end{fact}
...
Let $F$ be a field. If $I$ is any ideal of $F[x]$, then $I$ is a principal ideal. Moreover, if $I$ is not the zero ideal, then $I = (d(x))$ if and only if $d(x)$ has the smallest possible degree among all nonzero elements of $I$.
\end{fact}
...
\item Use Theorem~\ref{thm.FirstIsoRings} to show that the image of $\mathcal{E}_{i}:\mathbb{Q}[x]\rightarrow\mathbb{C}$ is a field, and then use Problem~\ref{prob.EvalAtI} to explain why $\operatorname{im} \mathcal{E}_{i}=\mathbb{Q}(i)$.
\item Conclude that $\mathbb{Q}[x]/(x^2+1) \cong \mathbb{Q}(i)$.
\end{enumerate}
\end{problem}
