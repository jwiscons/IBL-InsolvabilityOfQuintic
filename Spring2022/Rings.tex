...
\item $s(x) = (5-i)^5 - (5-s)^3$ in the ring $\mathbb{C}[s]$
\end{enumerate}
\end{problem}
...
Let $R$ be a ring. Suppose $p(x), q(x)\in R[x]$. If all of $\deg p(x)$, $\deg q(x)$, and $\deg(p(x) + q(x))$ are defined, then $\deg(p(x) + q(x)) \le \max(\deg p(x),\deg q(x))$.
\end{theorem}
...
Let $D$ be an integral domain. If $p(x)$ and $q(x)$ are nonzero polynomials $D[x]$, then $\deg(p(x)q(x)) = \deg p(x) + \deg q(x)$.
\end{theorem}
...
As with groups and fields, we will want to generate subobjects of rings from subsets. Generating ideals will be more useful for us that subrings, so we will only focus on ideals. Regarding notation, it is common to use $(A)$ for the ideal generated by $A$ instead of $\langle A \rangle$, and we will follow that convention. We begin with intersections.
