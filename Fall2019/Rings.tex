...
Let $R$ be a ring. If $p(x)$ and $q(x)$ are nonzero polynomials $R[x]$, then $\deg(p(x) + q(x)) \le \max(\deg p(x),\deg q(x))$ unless $q(x) = -p(x)$.
\end{theorem}
...
\begin{definition}
Let $F$ be a field, and let $a(x),b(x)\in F[x]$. We say that $b(x)$ \textbf{divides} $a(x)$ (or $b(x)$ is a \textbf{divisor} of $a(x)$) if there exists some $q(x)\in F[x]$ such that $a(x) = b(x)q(x)$.
\end{definition}
...
As is familiar, we will often treat polynomials as functions, and plug values into them. The next theorem should seem familiar.

\begin{theorem}\label{thm.LinearFactorOfPolyImpliesRoot}
Let $a(x)\in F[x]$ for $F$ a field. If $c\in F$ and $(x-c)$ divides $a(x)$, then $a(c) = 0$.
\end{theorem}
