...
\begin{definition}
Let $R$ be a ring. An element $a\in R$ is called a \textbf{zero divisor} if $a$ is nonzero and there exists another nonzero $b\in R$ such that $ab=0$. A ring is called an  \textbf{integral domain} if it is a commutative ring with unity containing at least two elements but \emph{no zero divisors}.
\end{definition}
...
Let's briefly explore the elements in a ring that do have a multiplicative inverse.
...
\item $C\in M_{2}\left(\mathbb{Q}\left(\sqrt{2}\right)\right)$ but $C\notin M_{2}(\mathbb{Q})$
...
The previous fact shows that $M_{n}(R)$ is indeed a ring with unity. Let's explore some of the other ring properties we discussed above.
...
\begin{problem}
Show that $\begin{bmatrix} i & 3\\ 0& i\end{bmatrix}\in\operatorname{GL}_{2}(\mathbb{C})$ by finding a multiplicative inverse for it.
\end{problem}
\end{section}
