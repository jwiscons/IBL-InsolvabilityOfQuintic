...
Let's explore the extra properties that a subring might need to ensure that the set of cosets can be given the structure of a ring. To start out, let's just assume that  $I$ is an \emph{additive subgroup} of the ring $R$. Since $(R,+)$ is abelian, $I$ is automatically a normal subgroup of $(R,+)$.  Now, let's consider the set of cosets of $I$ in $R$, which we write as $R/I$. Recall that $R/I = \{a + I\mid a\in R\}$ where $a+I = \{a+y\mid y\in I\}$. Here are a few properties about $R/I$ from group theory.
...
After distributing, this becomes $ab + I = ab + ay + xb + xy +I$, which simplifies to $I = ay + xb + xy +I$. By Fact~\ref{fact.AdditiveSubgroupRingQuotientProperties}, we see that what we really need to ensure is that 
...
Let's also summarize our work above as a fact.
...
$\{0,3,6,9,12\}\subset \mathbb{Z}_{12}$  &&& \\ \hline 
...
\item Explain why $I$ has polynomials of every degree larger than or equal to $2$.
\item Let $a(x) = x^4+3x+5$. Write out $5$ elements of the coset $a(x) + I$.
\item Find some $b(x)$ in $a(x) + I$ such that $\deg b(x) = 1$.
\item Explain why $(x + I)^2 = -1 + I$.
\end{enumerate}
\end{problem}
...
%
%\begin{theorem}
%If $I$ and $J$ are ideals of the ring $R$, then $I\cap J$ is an ideal of $R$.
%\end{theorem}
%\begin{definition}
