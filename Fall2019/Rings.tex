...
\item Explain why $(x + I)^2 = -1 + I$ in the ring $R/I$.
\end{enumerate}
\end{problem}
...
Let's record some observations from the previous problem.
...
As with groups and fields, we will want to generate sub-objects from subsets. Generating ideals will be more useful for us that subrings, so we will only focus on ideals. Regarding notation, it is common to use $(A)$ for the ideal generated by $A$ instead of $\langle A \rangle$, and we will follow that convention. We begin with intersections.
...
In fact, we can obtain  similar results in any ring with a suitable division algorithm, which is what we used to show the existence of a greatest common divisor. Importantly for us, this applies to polynomial rings over fields. Let's prove the result for ideals generated by two elements and leave the general case as a fact.
...
\begin{definition}
The \textbf{kernel} of a ring homomorphism $\phi:R\to S$ is defined to be $\ker(\phi)=\{a\in R\mid \phi(a)=0\}$. 
\end{definition}
