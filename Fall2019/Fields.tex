...
Notice that $\mathbb{Q}$, $\mathbb{R}$, and $\mathbb{C}$ satisfy many common algebraic properties with respect to addition and multiplication. Of course, $\mathbb{H}$ does too, though it lacks commutativity of multiplication. When objects have common properties, it can be extremely valuable to abstract the common structure, and study it once and for all (as opposed to trying to prove things about each individual structure). This is where we are headed, but first we highlight some related objects (again with similar  algebraic properties) that help to connect this work to our main goal of expressing roots of polynomials.
...
\item Use the fact that $S\subset \mathbb{R}$ to explain why addition and multiplication of elements of $S$ are both associative and commutative.
\end{enumerate}
\end{problem}
