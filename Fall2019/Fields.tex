...
The key point of the previous definition is that $i^2 = -1$. Using $i$ and $\mathbb{R}$, we now build the complex numbers.

\begin{definition}
The \textbf{complex numbers}, denoted  $\mathbb{C}$, is the set $\{a + bi\mid a,b \in \mathbb{R}\}$. If $z\in \mathbb{C}$ and $z$ can be written as $z=a+bi$, then $a$ is called the \textbf{real part} of $z$ and $b$ is called the \textbf{imaginary part} of $z$.
\end{definition}

Note that every complex number $z=a+bi$ is uniquely determined by two numbers: the real and imaginary parts $a$ and $b$. As such, we often graph complex numbers in the coordinate plane with the $x$-axis denoting the real part and the $y$-axis denoting the imaginary part.
...
Notice that in the definition of complex multiplication we are just using the normal distributative law (or FOIL if you like) together with the fact that $i^2 = -1$.

\begin{problem}
Thinking of a complex number $z$ as a point in the plane, describe \emph{geometrically} what happens when $(c+di)$ is added to $z$. Also, describe \emph{geometrically}  how to find $\overline{z}$ from $z$.
\end{problem}

When we plot points, there are different coordinate systems we could use. It turns out that the rectangular coordinates are good for adding complex numbers, but polar coordinates are better for multiplication. This lead to the following definition.
...
\item The \textbf{argument} of $z$, denoted $\Arg(z)$, is the angle of the point $(a,b)$ when written in polar coordinates. Thus, $\Arg(z)$ is the angle \emph{in the appropriate quadrant} whose tangent is $\frac{b}{a}$. The argument of  $0$ is undefined.
\end{enumerate}
\end{definition}
...
\begin{problem}
Write each of the following complex numbers in the form $a+bi$, and then find a decimal approximation of the modulus and argument.
\begin{enumerate}
\item $u= \frac{1}{1+i}$
\item $w = \frac{(2-i)(1+2i)}{2+3i}$
\item $z$ is the complex number with $|z| = 3$ and $\Arg(z) = \frac{4\pi}{3}$
\end{enumerate}
\end{problem}


theorems next (and reread previous work)
