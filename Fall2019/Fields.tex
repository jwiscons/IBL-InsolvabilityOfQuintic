...
We read $F(r_1,r_2,\ldots,r_n)$  as ``$F$ adjoin $r_1,r_2,\ldots,r_n$''; it is the smallest subfield containing $F$ and $r_1,r_2,\ldots,r_n$.

In the following problems, we are working with subfields of $\mathbb{C}$, even if we don't say it explicitly.
...
\begin{center}
\begin{tikzpicture}[line width = 1,yscale = .7]
...
\node[red] (root2) at (-2.5,-0.9) {$\sqrt{2}$};
\fill[red,anchor = 45,yscale = 10/7] (-2.8,-.9) circle (0.07);
\end{tikzpicture}
\end{center}
\end{problem}
