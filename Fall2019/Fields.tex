...
Notice that $\mathbb{Q}$, $\mathbb{R}$, and $\mathbb{C}$ satisfy many common algebraic properties with respect to addition and multiplication. Of course, $\mathbb{H}$ does too, though it lacks commutativity of multiplication. When objects have common properties, it can be extremely valuable to abstract the those properties and study them once and for all (as opposed to trying to prove things about each individual structure). This is where we are headed, but first we highlight some related structures (again with algebraic properties similar to $\mathbb{Q}$, $\mathbb{R}$, and $\mathbb{C}$) that help to connect this work to our main goal of expressing roots of polynomials.
...
\item Use that $S\subset \mathbb{R}$ to explain why addition and multiplication of elements of $S$ are both associative and commutative and why multiplication distributes over addition.
\end{enumerate}
\end{problem}
...
\begin{itemize}
\item \textbf{Addition Laws:} Addition is both associative and commutative; the element $0$ is an additive identity; every  $x\in F$ has an additive inverse with respect to $0$, denoted $-x$.
\item \textbf{Multiplication Laws:} Multiplication is both associative and commutative; the element $1$ is a multiplicative identity;  every $x\in F\setminus\{0\}$ has a multiplicative inverse with respect to $1$, denoted $x^{-1}$.
\item \textbf{Distributivity Laws:} For all $x,y,z \in \mathbb{F}$, $x(y+z) = xy+xz$ and $(y+z)x = yx+zx$.
\end{itemize}
\end{definition}
...
\begin{enumerate}
\item $(F,+,\cdot)$ with $+$ and $\cdot$ defined as follows:
\begin{center}
...
\begin{problem}
Look back at Problem~\ref{prob.FieldExampleTable}. For each structure that is a field, determine which familiar group each of $(F,+)$ and $(F^*,\cdot)$ is isomorphic to.
\end{problem}
...
\begin{problem}
Show that $\mathbb{Z}_3$ is a field but $\mathbb{Z}_4$ is not.
\end{problem}
...
Let's now explore some basic properties of fields that follow from the definition. We list some of these as facts since they follow directly from the fact that $(F,+)$ and $(F^*,\cdot)$ are both groups together with basic group theory. From now on, when we write ``let $F$ be a field,'' we tacitly mean ``let $(F,+,\cdot)$ be a field.''
...
Just as with groups, the notion of a subfield is extremely important. Analyzing the subfields of a field $F$ can often yield a better understanding of the whole field $F$, and vice versa. Also, of immediate use, it will allow us to generate many more examples of fields.
...
The second item in the above theorem is stating that $F$ is closed under the addition and multiplication of $E$. The last two items could be read as $F$ being closed additive and multiplicative inverses.

We now consider potential subfields of $\mathbb{C}$.

\begin{problem}
Let $T_1=\{a+bi\mid a,b\in \mathbb{Q}\}$. Show that $T_1$ is a subfield of $\mathbb{C}$.
\end{problem}

\begin{problem}
Let $T_2=\{a+bi\mid a,b\in \mathbb{Z}\}$. Show that $T_2$ is not subfield of $\mathbb{C}$.
\end{problem}

\begin{problem}\label{prob.SubfieldRoot2PlusI}
Let $\alpha = \sqrt{2} + i$, and define $T_3=\{a+b\alpha\mid a,b\in \mathbb{Q}\}$. Show that $T_3$ is not subfield of $\mathbb{C}$.
\end{problem}
