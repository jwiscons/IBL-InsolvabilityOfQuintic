...
\begin{problem}
Use Theorem~\ref{thm.HomFixingFPermutesRootsOfPolysOverF} to explain why each map defined below is \emph{not} be a homomorphism.
\begin{enumerate}
...
Let's use what we've learned so far to explore which subfields of $\mathbb{C}$ could be isomorphic to $\mathbb{Q}(\sqrt[3]{11})$ (just as an example). Let $\phi:\mathbb{Q}(\sqrt[3]{11})\to \mathbb{Q}(\beta)$ be an isomorphism---we'll try to find the possible values of $\beta$. Notice that Theorem~\ref{thm.HomFixingFPermutesRootsOfPolysOverF} may be helpful, but only if we know that $\phi$ fixes the coefficients of a polynomial that has $\sqrt[3]{11}$ as a root. Let's first show that $\phi$ must fix all of $\mathbb{Q}$. 
...
Let $\beta\in \mathbb{C}$, and assume that $\phi:\mathbb{Q}(\sqrt[3]{11})\to \mathbb{Q}(\beta)$ is an isomorphism.
...
\item What does this imply are the possibilities for $\mathbb{Q}(\beta)$?
\end{enumerate}
\end{problem}
...
As mentioned earlier, we really want to studying maps from a field to itself. We now down define one of the keys ingredients to our eventual solution to the insolvability of the quintic.

\begin{definition}
Let $K$ be a field. An isomorphism from $K$ to $K$ is called an \textbf{automorphism} of $K$. 
Define $\Aut(K)$ to be the set of all automorphisms of $K$, and if $F$ is a subfield of $K$, define $\Aut(K/F)$ to be the set of all automorphisms of $K$ that fix $F$.
\end{definition}

Unpacking the definition, we see that automorphisms of $K$ are  bijections from $K$ to $K$ (also called permutations of $K$) that are also homomorphisms. Since we know that the composition of two bijections is a bijections and the composition of two homomorphisms is a homomorphism (see Theorem~\ref{thm.CompositionHomomorphism}), we immediately arrive at the following fact.

\begin{fact}
Let $F$ be a subfield of $K$. Both of $\Aut(K)$ and $\Aut(K/F)$ are groups with respect to function composition. The identity is the identity function, which will be denoted id.
\end{fact}

\begin{example}\label{prob.AutQAdjoinRoot5OverQ}
Let's try to compute $\Aut(\mathbb{Q}(\sqrt{5})/\mathbb{Q})$. Recall that we know $\mathbb{Q}(\sqrt{5}) = \{a+b\sqrt{5}\mid a,b\in \mathbb{Q}\}.$
Thus, every function $\phi\in \Aut(\mathbb{Q}(\sqrt{5})/\mathbb{Q})$ is determined by a formula of the form \[\phi(a+b\sqrt{5}) = \text{ ??? }\]
Since $\phi\in \Aut(\mathbb{Q}(\sqrt{5})/\mathbb{Q})$, $\phi$ fixes $\mathbb{Q}$, so we can rewrite the above formula as 
\[\phi(a+b\sqrt{5}) = \phi(a)+\phi(b\sqrt{5}) = \phi(a)+\phi(b)\phi(\sqrt{5}) = a + b\phi(\sqrt{5}).\]
Thus, to find a formula for $\phi$ we just need to decide the value for $\phi(\sqrt{5})$. Now, we know that $\sqrt{5}$ is a root of $x^2-5$, so by Theorem~\ref{thm.HomFixingFPermutesRootsOfPolysOverF}, there are only two options: $\phi(\sqrt{5}) = \sqrt{5}$ or $\phi(\sqrt{5}) = -\sqrt{5}$. Let's name the possibilities: 
\begin{itemize}
\item $\phi_1$ sends $\sqrt{5}\mapsto \sqrt{5}$, so  $\phi_1(a + b\sqrt{5}) = a + b\sqrt{5}$;
\item $\phi_2$ sends $\sqrt{5}\mapsto -\sqrt{5}$, so  $\phi_1(a + b\sqrt{5}) = a - b\sqrt{5}$.
\end{itemize}
 Of course, $\phi_1$ is just the identity, so $\phi_1 \in \Aut(\mathbb{Q}(\sqrt{5})/\mathbb{Q})$. To check the other possibility, we can use Fact~\ref{fact.SameMinPolyIsomorphic}, which says that $\phi_2$ is an isomorphism from $\mathbb{Q}(\sqrt{5})$ to $\mathbb{Q}(-\sqrt{5})$. Since $\mathbb{Q}(-\sqrt{5}) =\mathbb{Q}(\sqrt{5})$, $\phi_2$ is indeed an automorphism of $\mathbb{Q}(\sqrt{5})$ (that fixes $\mathbb{Q}$). Thus, $\phi_2 \in \Aut(\mathbb{Q}(\sqrt{5})/\mathbb{Q})$. These are the only possibilities, so 
\[\Aut(\mathbb{Q}(\sqrt{5})/\mathbb{Q}) = \{\phi_1,\phi_2\} = \{\text{id},\phi_2\}.\]
And, since there is only one group of order $2$ (up to isomorphism), $\Aut(\mathbb{Q}(\sqrt{5})/\mathbb{Q}) \cong \mathbb{Z}_2$.
\end{example}

\begin{problem}\label{prob.AutQAdjoinZeta5OverQ}
Write down a formula for each function in $\Aut(\mathbb{Q}(\zeta_5)/\mathbb{Q})$. Can you determine which familiar group $\Aut(\mathbb{Q}(\zeta_5)/\mathbb{Q})$ is isomorphic to?
\end{problem}

\begin{problem}\label{prob.AutQAdjoinZeta5OverQ}
Use that $\mathbb{Q}(\sqrt[3]{11}) \subseteq \mathbb{R}$ while $\mathbb{Q}(\sqrt[3]{11}\zeta_3) \nsubseteq \mathbb{R}$ to show  $\Aut(\mathbb{Q}(\sqrt[3]{11})/\mathbb{Q}) = \{\text{id}\}$.
\end{problem}

We've seen that each element of $\Aut({F}(\alpha)/F)$ must take $\alpha$ to another root of the minimal polynomial for $\alpha$ over $F$. Moreover, for each each possible $\phi\in \Aut({F}(\alpha)/F)$, the value of $\phi(\alpha)$ can be use to completely describe $\phi$. Let's gather this up in a fact, together with a useful consequence.

\begin{fact}
Let $F$ be a subfield of $E$. Suppose that $\alpha\in E$ is algebraic over $F$, and let $n$ be the degree of $\alpha$ over $F$. Then each $\phi\in \Aut({F}(\alpha)/F)$ is completely determined by the value $\phi(\alpha)$, and consequently  $|\Aut({F}(\alpha)/F|\le n = [F(\alpha):F]$.
\end{fact}
