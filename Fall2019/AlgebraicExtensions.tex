...
\item Assume that $c_0+c_1\alpha+c_2\alpha^2+c_3\alpha^3+c_4\alpha^4 = 0$ for some $c_1,\ldots,c_4\in \mathbb{Q}$. Show that if at least one of  $c_1,\ldots,c_4$ is nonzero, then $\alpha$ is a root of some \emph{nonzero} polynomial that has degree at most $4$. 
...
Generalizing our work in Problem~\ref{prob.BasisQRootx55x45}, we obtain a crutial theorem.
...
\item Use Theorem~\ref{thm.BasisChainExtensionField} to find a basis for $\mathbb{Q}(\sqrt[4]{2},\zeta_3)$ over $\mathbb{Q}$.
\item What is $[\mathbb{Q}(\sqrt[4]{2},\zeta_3):\mathbb{Q}]$?
\end{enumerate}
\end{problem}
